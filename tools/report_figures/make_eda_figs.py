...
def fig_lorenz_targeting() -> Path:
    """لورنتس تمرکز هدررفت، با دو مرتب‌سازی متفاوت از یک جمعیت.

    منحنی بالایی افراد را با تعداد پرس هدررفته مرتب می‌کند و منحنی پایینی با نرخ شخصیِ
    کوچک‌سازی‌شده. چون تعداد حاصل‌ضرب نرخ در حجم رزرو است، مرتب‌سازی با تعداد پرمصرف‌ها را
    بالا می‌آورد و مرتب‌سازی با نرخ پرنرخ‌ها را؛ فاصله‌ی دو منحنی همین تفاوت است، نه
    تفاوت دقت. هر دو با کل داده حساب می‌شوند و ادعای خارج‌نمونه‌ای در کار نیست.
    ...
    ax.plot([0, 100], [0, 100], "k--", lw=1.2, label=fa("توزیع کاملاً برابر"))
    ax.plot(frac, cum_or, color=RED, lw=2.2, label=fa("مرتب‌سازی با تعداد پرس هدررفته"))
    ax.plot(frac, cum_eb, color=BLUE, lw=2.2, ls="-.",
            label=fa("مرتب‌سازی با نرخ کوچک‌سازی‌شده"))
    ...
    ax.annotate(fa(f"۱۰٪ بدترین‌ها: {s_or:.0f}٪ کل عدم‌دریافت"),
                ...
                arrowprops=dict(arrowstyle="->", color="gray", lw=0.9))
    ax.annotate(fa(f"با مرتب‌سازی بر نرخ: {s_eb:.0f}٪"),
