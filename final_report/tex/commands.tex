...
% این خط به‌تنهایی کافی نیست: xepersian.sty خودش \bibname را در \AtBeginDocument به
% «کتاب‌نامه» برمی‌گرداند و آن هوک همیشه بعد از این خط (که هنگام بارگذاری فایل، نه در
% \AtBeginDocument، اجرا می‌شود) فعلی‌سازی می‌شود، پس آخرین‌بار برنده است. راه‌حل: همین
% تغییر را هم در \AtBeginDocument خودمان بگذاریم — چون بعد از xepersian در main.tex
% \input شده، هوک ما در صف \AtBeginDocument دیرتر ثبت و اجرا می‌شود و برنده می‌ماند.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%
% جداکننده‌ی شماره‌ی فصل از شماره‌ی شکل/جدول/رابطه. پیش‌فرض زی‌پرشین «.» است ولی فهرست
% مطالب رسمی قالب دانشگاه («شکل (۴-۱) …»، «جدول (۴-۲) …») خط‌فاصله می‌خواهد — قرارداد
% نگارش در final_report/AGENTS.md. بدون این خط عنوان‌ها «شکل ۴.۱» می‌شوند.
\SepMark{-}
...
% رابطه‌ها شماره‌گذاری **سراسری** دارند («رابطه (۱)»، «رابطه (۲)» …)، نه فصلی —
% قرارداد نگارش در final_report/AGENTS.md. کلاس از book ارث می‌برد و book شمارنده‌ی
% equation را با هر فصل صفر می‌کند و «۴-۲» می‌سازد؛ خط زیر همین را خنثی می‌کند.
...
% ⚠️ عمداً به کد اصلی قالب برگردانده شد — توضیح در final_report/AGENTS.md، بخش
% «مراجع: چرا شماره‌دار نیست». خلاصه: بازتعریف خام \thebibliography (چه با این نسخه‌ی
% اصلی، چه با تلاش‌های اصلاحی این جلسه) با پیاده‌سازی تگ‌دار محیط‌ها در نسخه‌ی مدرن
% هسته‌ی LaTeX2e این TeXLive ناسازگار است و با ارجاع واقعی خطای فاتال می‌دهد
% («\begin{thebibliography}...ended by \end{thebibitemlist}»). چون رفعش نیازمند
% دستکاری عمیق هسته بود (فراتر از رفع باگ یک قالب vendored)، فصل مراجع را با
% مکانیزم پیش‌فرض \bibliography (کار می‌کند، امتحان‌شده) بدون دست‌زدن به
% \thebibliography رها کردیم — یعنی بدون شماره‌ی «فصل ۶:» و با نام پیش‌فرض
% «کتاب‌نامه» (نه «مراجع» — رفعش هم به همین دلیل با \AtBeginDocument پایین‌تر
% انجام شده، نه اینجا).
\makeatletter
