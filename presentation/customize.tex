...
% usefonttheme{serif} غیرفعال شد: با بسته‌ی beamerfontthemeserif که xepersian خودش
% از قبل بار می‌کند تداخل می‌کند و کامپایل را می‌شکند (Option clash)
% \usefonttheme{serif}

% TYPESETTING ELEMENTS

% style of section presented in the table of contents
% با xepersian، رنگ‌گذاری خودکار پویای «section in toc / shaded» (که بین سکشن جاری و
% بقیه تمایز رنگی ایجاد می‌کند) از کار می‌افتد چون بستهٔ داخلی xepersian هنگام تایپ‌ست
% کردن عنوان‌های انگلیسی سکشن رنگ محیطی را ریست می‌کند و \color ساده اثر نمی‌کند —
% \textcolor (که رنگ را با گروه محلی خودش اعمال می‌کند) از این ریست جان سالم به در می‌برد،
% پس هر دو حالت اینجا صریحاً با \textcolor تنظیم شده‌اند
\setbeamertemplate{section in toc}{\textcolor{white}{$\blacktriangleright$~\inserttocsection}}
\setbeamertemplate{section in toc shaded}{\textcolor{white!40!sintefblue}{$\blacktriangleright$~\inserttocsection}}
